...
q_SE3_1 = q_SE3;% zeros(N_steps,6);
dq_SE3_1 = dq_SE3;% zeros(N_steps,6);
...
for k = Start+1:N:N_steps-1
    ...
    input = [q_leg(k,:)'+ randn(14,1) * encoder_noise;dq_leg(k,:)'+ randn(14,1) * encoder_noise;u(k,:)';contact(k,:)';dt];
    ...
    %%
    xd = q_SE3_1(k,1:6)' + x_hat(7:12) * dt + ud(1:6);
...
for k = 1:6
    ...
    hold on
    if k < 40
        plot(Time(1:N:end),q_SE3_1(1:N:length(Time),k),'b')
